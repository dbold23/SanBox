# Prototype 04 on the real Meshy GLB, after the fixes (final run v11, 2026-09-02)
...
identical).  Same command as before, `--up 0 1 0`, plus `--caudal-lobes auto --fin-blend-dist 0.012`.  Neither GLB is committed (123 MB / 153 MB in,
171 MB out).
...
| slivers tearing off the tail mid-beat | the caudal lobe (24 % of the body, a smooth continuation of the peduncle) was driven as a fin about a root joint: a mid-lobe root levers its front half against the body, a base root shears its underside | a caudal island that is a lobe (axial extent >= 2x radial, here 4.4x) is folded into the body and the spine polyline is extended from the chart's end to the lobe's tip (0.480 -> 0.666 m, precaudal fraction rescaled 0.78 -> 0.563 so the precaudal joint stays at the peduncle), so the two caudal-axis joints sit on the lobe and the wave continues into it; the as_scanned solve carries the real rest frame of those bones (`--caudal-lobes auto`; the default `fins` keeps the synthetic fixture unchanged) |
| tail too stiff after the fold (the whole lobe swung as one piece off the last joint) | no joints in the lobe | same spine extension: caudal_axis_1/2 now lie on the lobe |
| pectoral (and other) fins looked detached at the root under the fin drive: a slot along the base | the fin-base weight blend was 3 mesh edge rings, ~1.5 mm on this 1M-vertex mesh, so the blade (fin joints) pulled away from the root ring (spine joints) | distance-based blend: body vertices within 12 mm of a fin island get smoothstep root weight (`--fin-blend-dist 0.012`, `rig.compute_weights(fin_blend_dist=)`) |
...
| quantity | before (`results/real`) | after (`results/real_v11`) |
|---|---|---|
...
| joints | 37 | 33 (13 spine + 20 fin); the caudal lobe is body and the last two spine joints lie on it |
| as_scanned spine joint error max | 0.00118 m | 0.00361 m (the two lobe joints target the scanned lobe segment) |
...
`rig_run.log`, `check.log`.  The viewer at `results/real/viewer/index.html` takes `?glb=../v11/sevengill_rigged.glb`.
